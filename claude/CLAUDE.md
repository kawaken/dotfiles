...
- ExitPlanModeを呼ぶ前に、必ず `/gtgt` スキルを実行してPlanを煮詰めること
  - `/gtgt` が完了した後、煮詰まったPlanの内容をユーザーに提示してからExitPlanModeを呼ぶ
...
- Bashコマンド内で複雑なクォーティング（クォートのネストや `\n` 等のエスケープを含むフォーマット文字列）を避ける
  - 例: `gh api --jq` のフィルター内に `"...\n...\(.field)..."` のようなフォーマット文字列をインラインで書かない
  - 対策: パイプで分離する（`gh api ... | jq -r '...'`）か、フォーマット文字列を使わずシンプルなフィールド抽出にする
